...
empleados_archivo = "empleados.csv"
...
class Empleados():
    ...
    def imprimirT(self):
        print(f"ID:{self.id_empleado} Nombre: {self.nombre} Edad:{self.edad} Genero:{self.genero}")
        print(f"Puesto:{self.puesto} Cargo:{self.clientes_cargo} Estatus:{self.estaHabilitado}")
        print("----------------------------------------------------------------------------------------\n")
    

class Clientes():
    def _init_(self, id_cliente, nombre_clientes, rfc, domicilio, contacto, nombre_responsable):
    ...
    def imprimirCliente(self):
        ...
        print(f"Domicilio:{self.domicilio} - Contacto:{self.contacto} - Nombre Responsable:{self.nombre_responsable}")
        print("----------------------------------------------------------------------------------------\n")
...
class Productos():
    def __init__(self, id_producto,nombre_p, stock, clave_sat, precio_renta, precio_costo):
        ...
        self.tieneIVA = True
    def ImprimirP(self):
        print(f"ID:{self.id_producto} Nombre:{self.nombrep}  Stock:{self.stock}   Precio:{self.precio_costo}")
...
class Venta():
    def __init__(self, id_venta, fecha, folio, id_cliente,total):
        ...
        self.total = total
    def imprimirV(self):
        print(f"ID:{self.id} fecha:{self.fecha} Folio:{self.folio}   Total={self.total}")
...
class Empresa():
    ...
    def actualizar_empleado(self):
        ...
        file = open(empleados_archivo, mode='r', newline='')
        with file as empleados_archivos:
            csv_reader = csv.reader(empleados_archivos, delimiter=',')
            for row in csv_reader:
                temp = Empleados(row[0], row[1], row[2], row[3], row[4])
    ...
    def actualizar_producto(self):
        ...
        with file as producto_archivos:
            ...
            for row in csv_reader:
                temp = Productos(row[0], row[1], row[2], row[3], row[4], row[5])
    ...
    def Ventas(self):
        ...
        idk= input("Ingresa tu nombre en el registro de clientes")
        file = open(cliente_archivo, mode='r', newline='')
        with file as clients:
            csv_reader = csv.reader(cliente_archivo, delimiter=',')
            for row in csv_reader:
                if idk==row[0]:
                    print("Bienvenido al portal de clientes")
                else:
                    break
        Facturas.Ver_productos()
        # while(True):
        #     id_pr=int(input("ID del Producto que compro:"))
        #     file = open(producto_archivo, mode='r', newline='')
        #     with file as producto_archivos:
        #         csv_reader = csv.reader(producto_archivos, delimiter=',')
        #         for row in csv_reader:
...
while(True):
    ...
    if(opcion == 1):
        ...
        genero = input("Genero: ")
        puesto = input("Puesto: ")
        i = Empleados(id_global_empleado, nombre, edad, genero, puesto, clientes_cargo)
       ...
       input("\nPresione enter para continuar ...")
        ...
        if os.path.exists(producto_archivo):
            ...
            if filesize == 0:
                ...
                with file as producto_archivos:
                    ...
                    cliente_lectura.writerow ([id_p,nombre_pro,stock,Sat,precio_renta,precio_costo])
            file.close()
        else:
        ...
        if os.path.exists(cliente_archivo):
            ...
            if filesize == 0:
                ...
                file = open(cliente_archivo, mode='r+', newline='')
                with file as cliente_archivos:
                    id = max(int(line.split(',')[0]) for line in cliente_archivos)+1
                    ...
                    cliente_lectura = csv.writer(cliente_archivos, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                    cliente_lectura.writerow([id, nombre_clientes, rfc, domicilio, contacto, nombre_responsable, today])
            file.close()
        else:
            ...
            with file as cliente_archivos:
                ...
                cliente_lectura.writerow([id, nombre_clientes, rfc, domicilio, contacto, nombre_responsable, today])
            file.close()
       # producto=Facturas.Ventas()
        #vent=Venta(id_global_venta,today,id_global_venta,id,producto)
        #Facturas.ImpV()
        print("---------------------------------------------------")
        input("\nPresione enter para continuar ...")
    elif(opcion == 8):
